...
<section id="main_section">
	...
	<section class="album_section">
		...
		<?php foreach ($rapalbums as $rapalbum) : ?>
			...
			<figure class="grid_1_of_5 ">
				<a href="/singleresult?albumid=<?= $rapalbum->id ?? " " ?>&artistid=<?= $rapalbum->artistId ?? '' ?>" title="<?= $rapalbum->getArtistId()->artist_name ?>">
	...
	<section class="album_section ">
		...
		<?php foreach ($countryalbums as $countryalbum) : ?>
			...
			<figure class="grid_1_of_5">
				<a href="/singleresult?albumid=<?= $countryalbum->id ?? '' ?>&artistid=<?= $countryalbum->artistId ?? '' ?>" title="<?= $countryalbum->getArtistId()->artist_name ?>">
					<img class="images_1_of_5" src="/assets/databasepics/WEBP/<?= $countryalbum->image ?>" alt="CountryMusicAlbum">
				</a>
				<figcaption class="cap_1_of_5">
	...
	<section class="album_section">
		...
		<?php foreach ($jazzalbums as $jazzalbum) : ?>
			...
			<figure class="grid_1_of_5">
				<a href="/singleresult?albumid=<?= $jazzalbum->id ?? '' ?>&artistid=<?= $jazzalbum->artistId ?? '' ?>" title="<?= $jazzalbum->getArtistId()->artist_name ?>">
					<img class="images_1_of_5" src="/assets/databasepics/WEBP/<?= $jazzalbum->image ?>" alt="JazzMusicAlbum">
				</a>
				<figcaption class="cap_1_of_5">
